Class 15: Transcriptonomics and analysis of RNA-Seq data
================
...
First task- find which columns in **counts** correspond to the control conditions (no drug) Then calculate the mean value for each gene.
...
note: the NaN (“not a number””) and -Inf (negative infinity) results NaN is returned when you divide by zero and try to take the log -Inf is returned when you try to take log of zero. It turns out that there are a lot of genes with zero expression...filter data to remove these genes.
